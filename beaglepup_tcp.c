...
void interrupt_handler()
{
    ...
    sprintf(report_buf, "%.2d:%.2d:%.2d SHUTDOWN",
            local_time->tm_hour,
            local_time->tm_min,
            local_time->tm_sec);
    dprintf(sockfd, "%s\n", report_buf);
...
{
    // get options
    char usage[] = "usage: ./lab4c_tcp portnum [--period=#] [--scale=C/F] --log=<filename> --id=<UID> --host=<host>";
    int required = 0;
    char * scale;
    int opt;
    ...
    while(run_flag)
    {
        ...
        if (report) // report
        {
            sprintf(report_buf, "%.2d:%.2d:%.2d %.1f",
                    local_time->tm_hour,
                    local_time->tm_min,
                    local_time->tm_sec,
                    temp);
